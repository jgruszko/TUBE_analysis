{
  const char* histfile = "./hist_DS40_0.root";
  const char* histname = "trapEMPZ";
  const char* templatefile = "BkgFitTemplates.root";
  //const char* templatename = "fitHighStats";
  const char* templatename = "fitLowStats";
  const char* outputfile = "fitResults_DS40_0.root";
  ...
  cout << "Calibration parameters: " << cal0 << ' ' << cal1 << endl;
	...
	calMap.AddEntryToVectors(112, histname, 2060, 2245);
	calMap.SetScale(112, histname,  2060, 2245, cal1);
	calMap.SetOffset(112, histname, 2060, 2245, cal0);
